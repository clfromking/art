...
Page({
    ...
    wx.getStorage({
      ...
      success: function (res) {
        ...
        for (var i = 0; i < gift_lists.length; i++) {
          ...
          gifts_total += Number(gift_lists[i].choosenum)
          gift_lists[i].index = i
          money += Number(gift_lists[i].choosenum) * Number(gift_lists[i].price)
          gift_lists[i].choosenum = '×' + gift_lists[i].choosenum + '件'
        }
        that.setData({
          gift_lists: gift_lists,
  ...
  addOrder: function () {
    ...
    var postData = { 'uid': uid, 'shop_id': waitOperateGifts.selectids.join(','), 'shop_num': waitOperateGifts.selectorderNums.join(','), 'order_id': waitOperateGifts.orderids.join(','),'way':3}
